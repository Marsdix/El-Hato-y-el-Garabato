// Seed: crea o reemplaza los posts de blog en Sanity.
// Ejecutar desde studio/:
//   SANITY_AUTH_TOKEN=skTuToken node scripts/seed-blog.mjs
//
// Nota: el campo "imagen" es un asset de Sanity y no puede importarse desde
// URLs externas. Añade las imágenes manualmente desde el Studio (localhost:3333).
...
// Añade _key a cada elemento de un array (obligatorio en Sanity)
function keyed(arr) {
  return arr.map((item, i) => ({ _key: `k${i}`, ...item }))
}

// Convierte los items de un bloque lista añadiendo _key
...
async function seed() {
  ...
  console.log('\nListo. Añade las imágenes de portada desde el Studio en localhost:3333')
}
